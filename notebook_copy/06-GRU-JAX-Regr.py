...
@app.cell
def _(X, jnp, timesteps, y):
    # Get Sample Numbers
    num_rows = X.shape[0]

    # Find the Batches
    batches = [
        (X[idx : idx + timesteps, :], y[idx + timesteps, :])
...
@app.cell
def _(go, train_history):
    target = train_history[-1][2].squeeze()  # Target
    pred = train_history[-1][1].squeeze()  # Prediction
